...
public class VideoToFrames implements Runnable {
    ...
    public void videoDecode(String videoFilePath) {
        ...
        try {
            ...
            if (isColorFormatSupported(decoder.getCodecInfo().getCapabilitiesForType(mime))) {
    ...
    private boolean isColorFormatSupported(MediaCodecInfo.CodecCapabilities caps) {
        for (int c : caps.colorFormats) {
            if (c == 2135033992) {
                return true;
            }
                ...
                if (doRender) {
                    ...
                    if (play_surf == null) {
                        ...
                        if (outputImageFormat != null) {
                            HookMain.data_buffer = getDataFromImage(image);
                        }
                        if (image != null) {
                            image.close();
                        }
